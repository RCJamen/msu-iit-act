...
tr_c :: Com -> Code
tr_c (Coms xs)      = concatMap tr_c xs
tr_c (Skip)         = [NOOP]
tr_c (Assign m v)   = tr_a v ++ [STORE m]
tr_c (If b c1 c2)   = tr_b b ++ [BRANCH(tr_c c1, tr_c c2)]
tr_c (WhileDo b c)  = [LOOP (tr_b b, tr_c c)]
tr_c (DoWhile c b)  = tr_c c ++ [LOOP (tr_b b, tr_c c)]
tr_c (RepUntil c b) = tr_c c ++ [LOOP (tr_b b ++ [NOT], tr_c c)]
tr_c (ForLoop (c1, b, c2) c3) = tr_c c1 ++ [LOOP (tr_b b, (tr_c c3 ++ tr_c c2))]
