...
to go ;; each tick of simulation turtle do this
  ...
  [
    move
    if count turtles-here with [color = yellow] > 0 and random 99 < transmission-rate
    [set color black]
  ]
  ask turtles [spread]
  ask turtles [recover]
...
to recover ;; if our agent has virus, he con recover under probability and after some virus_duration but he has also a chance to die
  if virus? [
    set virus_duration virus_duration + .3 ;;  removal rate
    if random-float 1.0 < recover_probability and virus_duration > 10 [ ;; chance to recover // incubation rate
      set virus? false
      set immune true
    if random-float 1.0 < death_probability and virus_duration > 10 [ ;; chance to die
        set dead true
      ]
    ]
  ]
...
to recolor ;; recolors agent based on their virus? state [red-infected, blue - not infected ]
  ifelse virus? [set color red] [set color yellow]
...
"" ""
PENS
"pen-1" 1.0 0 -7500403 true "" "plot count turtles with [dead]"
"pen-2" 1.0 0 -1184463 true "" "plot number_turtles - count turtles with [dead]"
"pen-3" 1.0 0 -2674135 true "" "plot count turtles with [virus?]"
"pen-4" 1.0 0 -10899396 true "" "plot count turtles with [immune]"
